...
export const peers = [
  { name: 'KUA',                          type: 'boarding-secondary', isUs: true, provenance: 'cited', sinksQuantified: true,
    ...
    offsets: 0,
    note: `Preliminary per-student figures from KUA gross/sinks ÷ ${TOTAL_STUDENTS} enrolled students (Wikipedia + KUA "By the Numbers"). Scope 1 = ${KUA_SCOPE1_TOTAL_MT.toLocaleString()} mt heating fuel + refrigerants + fleet. Scope 2 = ${Math.round(KUA_SCOPE2_ANNUAL_MT).toLocaleString()} mt — Year 1 projection from BMS-measured kWh × ISO-NE 2024 per-fuel factors (${GRID_MIX_TOTAL_MTCO2E.toFixed(1)} mt YTD seasonally extrapolated). Scope 3 = ${KUA_SCOPE3_TOTAL_MT.toLocaleString()} mt — dominated by international + US-boarder term-break travel. Sinks = ${Math.round(ANNUAL_SEQUESTRATION_MT).toLocaleString()} mt from ~1,000 acres of campus forest (campus is 1,300 acres total; ~1,000 forested) on a net basis, inside a ${SINKS_RECONCILIATION.methodCount}-method range (${SINKS_RECONCILIATION.lowMt.toLocaleString()}–${SINKS_RECONCILIATION.highMt.toLocaleString()}) and above its central of ${SINKS_RECONCILIATION.centralMt.toLocaleString()}, because the other methods average in harvested acres and KUA does not harvest — which matters here, because the sink is what puts KUA ahead of its peers.` },
...
function buildLivePeers(live, s2) {
  return peers.map((p) => {
    ...
    return {
      ...
      note: `Preliminary per-student figures from KUA gross/sinks ÷ ${TOTAL_STUDENTS} enrolled students (Wikipedia + KUA "By the Numbers"). Scope 1 = ${Math.round(scope1).toLocaleString()} mt heating fuel + refrigerants + fleet. Scope 2 = ${Math.round(scope2).toLocaleString()} mt — Year 1 projection from BMS-measured kWh × ISO-NE 2024 per-fuel factors (${ytdMt.toFixed(1)} mt YTD seasonally extrapolated). Scope 3 = ${Math.round(scope3).toLocaleString()} mt — dominated by international + US-boarder term-break travel. Sinks = ${Math.round(ANNUAL_SEQUESTRATION_MT).toLocaleString()} mt from ~1,000 acres of campus forest (campus is 1,300 acres total; ~1,000 forested) on a net basis, inside a ${SINKS_RECONCILIATION.methodCount}-method range (${SINKS_RECONCILIATION.lowMt.toLocaleString()}–${SINKS_RECONCILIATION.highMt.toLocaleString()}) and above its central of ${SINKS_RECONCILIATION.centralMt.toLocaleString()}, because the other methods average in harvested acres and KUA does not harvest — which matters here, because the sink is what puts KUA ahead of its peers.`,
    };
  });
}

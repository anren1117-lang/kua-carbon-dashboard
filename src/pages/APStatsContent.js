...
import { APSTATS_UNITS } from '../data/ap-content/apstats.js';
import { APSTATS_FIGURES } from '../data/ap-content/apstats-figures.js';
...
// AP Statistics — full teaching content viewer.
...
export default function APStatsContent() {
  return (
    <PasswordGate
      title="AP Statistics"
...
function APStatsViewer() {
  ...
  const unit = useMemo(
    () => APSTATS_UNITS.find((u) => u.number === selectedUnitNum) || APSTATS_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APSTATS_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Statistics — full content"
      subtitle={`${APSTATS_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Statistics', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apstats/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APSTATS_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apstats', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apstats', subunit)} />}

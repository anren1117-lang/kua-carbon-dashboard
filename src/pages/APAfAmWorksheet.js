...
export default function APAfAmWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APAFAM_UNITS.find((u) => String(u.number) === String(unitNum)) || APAFAM_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apafam" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP African American Studies · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP African American Studies · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

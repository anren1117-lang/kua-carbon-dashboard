...
import { APPHYS1_UNITS } from '../data/ap-content/apphys1.js';
import { APPHYS1_FIGURES } from '../data/ap-content/apphys1-figures.js';
...
// AP Physics 1 — full teaching content viewer.
...
export default function APPhys1Content() {
  return (
    <PasswordGate
      title="AP Physics 1"
...
function APPhys1Viewer() {
  ...
  const unit = useMemo(
    () => APPHYS1_UNITS.find((u) => u.number === selectedUnitNum) || APPHYS1_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APPHYS1_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Physics 1 — full content"
      subtitle={`${APPHYS1_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Physics 1', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apphys1/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APPHYS1_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apphys1', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apphys1', subunit)} />}

...
import { APMUSIC_UNITS } from '../data/ap-content/apmusic.js';
import { APMUSIC_FIGURES } from '../data/ap-content/apmusic-figures.js';
...
// AP Music Theory — full teaching content viewer.
...
export default function APMusicContent() {
  return (
    <PasswordGate
...
function APMusicViewer() {
  ...
  const unit = useMemo(
    () => APMUSIC_UNITS.find((u) => u.number === selectedUnitNum) || APMUSIC_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APMUSIC_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Music Theory — full content"
      subtitle={`${APMUSIC_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Music Theory', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apmusic/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APMUSIC_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apmusic', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apmusic', subunit)} />}

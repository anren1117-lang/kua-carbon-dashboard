...
import { APMACRO_UNITS } from '../data/ap-content/apmacro.js';
import { APMACRO_FIGURES } from '../data/ap-content/apmacro-figures.js';
...
// AP Macroeconomics — full teaching content viewer.
...
export default function APMacroContent() {
  return (
    <PasswordGate
...
function APMacroViewer() {
  ...
  const unit = useMemo(
    () => APMACRO_UNITS.find((u) => u.number === selectedUnitNum) || APMACRO_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APMACRO_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Macroeconomics — full content"
      subtitle={`${APMACRO_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Macroeconomics', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apmacro/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APMACRO_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apmacro', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apmacro', subunit)} />}

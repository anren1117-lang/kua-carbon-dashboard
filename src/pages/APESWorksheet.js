...
export default function APESWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APES_UNITS.find((u) => String(u.number) === String(unitNum)) || APES_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apes" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Environmental Science · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Environmental Science · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

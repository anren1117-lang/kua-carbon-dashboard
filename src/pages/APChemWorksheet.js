...
export default function APChemWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APCHEM_UNITS.find((u) => String(u.number) === String(unitNum)) || APCHEM_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apchem" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Chemistry · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Chemistry · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

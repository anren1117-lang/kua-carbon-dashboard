...
import { APSPANISH_UNITS } from '../data/ap-content/apspanish.js';
import { APSPANISH_FIGURES } from '../data/ap-content/apspanish-figures.js';
...
// AP Spanish Language and Culture — full teaching content viewer.
...
export default function APSpanishContent() {
  return (
    <PasswordGate
...
function APSpanishViewer() {
  ...
  const unit = useMemo(
    () => APSPANISH_UNITS.find((u) => u.number === selectedUnitNum) || APSPANISH_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APSPANISH_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Spanish Language and Culture — full content"
      subtitle={`${APSPANISH_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Spanish Language and Culture', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apspanish/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APSPANISH_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apspanish', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apspanish', subunit)} />}

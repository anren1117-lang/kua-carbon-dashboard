...
import { APCHEM_UNITS } from '../data/ap-content/apchem.js';
import { APCHEM_FIGURES } from '../data/ap-content/apchem-figures.js';
...
// AP Chemistry — full teaching content viewer.
...
export default function APChemContent() {
  return (
    <PasswordGate
      title="AP Chemistry"
...
function APChemViewer() {
  ...
  const unit = useMemo(
    () => APCHEM_UNITS.find((u) => u.number === selectedUnitNum) || APCHEM_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APCHEM_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Chemistry — full content"
      subtitle={`${APCHEM_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Chemistry', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apchem/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APCHEM_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apchem', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apchem', subunit)} />}

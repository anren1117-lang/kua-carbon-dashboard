...
export default function APMusicWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APMUSIC_UNITS.find((u) => String(u.number) === String(unitNum)) || APMUSIC_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apmusic" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Music Theory · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Music Theory · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

...
import { APHUG_UNITS } from '../data/ap-content/aphug.js';
import { APHUG_FIGURES } from '../data/ap-content/aphug-figures.js';
...
// AP Human Geography — full teaching content viewer.
...
export default function APHugContent() {
  return (
    <PasswordGate
...
function APHugViewer() {
  ...
  const unit = useMemo(
    () => APHUG_UNITS.find((u) => u.number === selectedUnitNum) || APHUG_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APHUG_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Human Geography — full content"
      subtitle={`${APHUG_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Human Geography', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/aphug/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APHUG_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('aphug', subunit)) && <VideoBlock video={subunit.video || resolveVideo('aphug', subunit)} />}

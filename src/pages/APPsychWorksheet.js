...
export default function APPsychWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APPSYCH_UNITS.find((u) => String(u.number) === String(unitNum)) || APPSYCH_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/appsych" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Psychology · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Psychology · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

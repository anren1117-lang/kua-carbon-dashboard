...
import { APRESEARCH_UNITS } from '../data/ap-content/apresearch.js';
import { APRESEARCH_FIGURES } from '../data/ap-content/apresearch-figures.js';
...
// AP Research — full teaching content viewer.
...
export default function APResearchContent() {
  return (
    <PasswordGate
      title="AP Research"
...
function APResearchViewer() {
  ...
  const unit = useMemo(
    () => APRESEARCH_UNITS.find((u) => u.number === selectedUnitNum) || APRESEARCH_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APRESEARCH_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Research — full content"
      subtitle={`${APRESEARCH_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Research', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apresearch/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APRESEARCH_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apresearch', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apresearch', subunit)} />}

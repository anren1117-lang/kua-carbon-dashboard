...
import { APWORLD_UNITS } from '../data/ap-content/apworld.js';
import { APWORLD_FIGURES } from '../data/ap-content/apworld-figures.js';
...
// AP World History — full teaching content viewer.
...
export default function APWorldContent() {
  return (
    <PasswordGate
...
function APWorldViewer() {
  ...
  const unit = useMemo(
    () => APWORLD_UNITS.find((u) => u.number === selectedUnitNum) || APWORLD_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APWORLD_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP World History — full content"
      subtitle={`${APWORLD_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP World History', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apworld/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APWORLD_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apworld', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apworld', subunit)} />}

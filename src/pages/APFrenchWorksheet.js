...
export default function APFrenchWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APFRENCH_UNITS.find((u) => String(u.number) === String(unitNum)) || APFRENCH_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apfrench" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP French Language and Culture · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP French Language and Culture · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

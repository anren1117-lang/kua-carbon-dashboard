...
export default function APStatsWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APSTATS_UNITS.find((u) => String(u.number) === String(unitNum)) || APSTATS_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apstats" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Statistics · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Statistics · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

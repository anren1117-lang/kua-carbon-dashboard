...
export default function APCompWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APCOMP_UNITS.find((u) => String(u.number) === String(unitNum)) || APCOMP_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apcomp" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Comparative Government · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Comparative Government · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

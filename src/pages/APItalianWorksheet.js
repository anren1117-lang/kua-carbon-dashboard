...
export default function APItalianWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APITALIAN_UNITS.find((u) => String(u.number) === String(unitNum)) || APITALIAN_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apitalian" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Italian Language and Culture · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Italian Language and Culture · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

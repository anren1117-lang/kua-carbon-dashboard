...
export default function APCSAWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APCSA_UNITS.find((u) => String(u.number) === String(unitNum)) || APCSA_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apcsa" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Computer Science A · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Computer Science A · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

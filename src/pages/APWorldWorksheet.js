...
export default function APWorldWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APWORLD_UNITS.find((u) => String(u.number) === String(unitNum)) || APWORLD_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apworld" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP World History · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP World History · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

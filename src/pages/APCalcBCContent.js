...
import { APCALCBC_UNITS } from '../data/ap-content/apcalcbc.js';
import { APCALCBC_FIGURES } from '../data/ap-content/apcalcbc-figures.js';
...
// AP Calculus BC — full teaching content viewer.
...
export default function APCalcBCContent() {
  return (
    <PasswordGate
...
function APCalcBCViewer() {
  ...
  const unit = useMemo(
    () => APCALCBC_UNITS.find((u) => u.number === selectedUnitNum) || APCALCBC_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APCALCBC_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Calculus BC — full content"
      subtitle={`${APCALCBC_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Calculus BC', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apcalcbc/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APCALCBC_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apcalcbc', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apcalcbc', subunit)} />}

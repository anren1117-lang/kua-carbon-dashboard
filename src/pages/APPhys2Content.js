...
import { APPHYS2_UNITS } from '../data/ap-content/apphys2.js';
import { APPHYS2_FIGURES } from '../data/ap-content/apphys2-figures.js';
...
// AP Physics 2 — full teaching content viewer.
...
export default function APPhys2Content() {
  return (
    <PasswordGate
      title="AP Physics 2"
...
function APPhys2Viewer() {
  ...
  const unit = useMemo(
    () => APPHYS2_UNITS.find((u) => u.number === selectedUnitNum) || APPHYS2_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APPHYS2_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Physics 2 — full content"
      subtitle={`${APPHYS2_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Physics 2', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apphys2/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APPHYS2_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apphys2', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apphys2', subunit)} />}

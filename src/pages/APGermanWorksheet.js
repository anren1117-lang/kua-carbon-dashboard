...
export default function APGermanWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APGERMAN_UNITS.find((u) => String(u.number) === String(unitNum)) || APGERMAN_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apgerman" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP German Language and Culture · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP German Language and Culture · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

...
import { APPHYSCMECH_UNITS } from '../data/ap-content/apphyscmech.js';
import { APPHYSCMECH_FIGURES } from '../data/ap-content/apphyscmech-figures.js';
...
// AP Physics C: Mechanics — full teaching content viewer.
...
export default function APPhysCMechContent() {
  return (
    <PasswordGate
...
function APPhysCMechViewer() {
  ...
  const unit = useMemo(
    () => APPHYSCMECH_UNITS.find((u) => u.number === selectedUnitNum) || APPHYSCMECH_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APPHYSCMECH_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Physics C: Mechanics — full content"
      subtitle={`${APPHYSCMECH_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Physics C: Mechanics', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apphyscmech/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APPHYSCMECH_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apphyscmech', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apphyscmech', subunit)} />}

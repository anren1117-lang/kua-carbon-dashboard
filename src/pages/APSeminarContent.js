...
import { APSEMINAR_UNITS } from '../data/ap-content/apseminar.js';
import { APSEMINAR_FIGURES } from '../data/ap-content/apseminar-figures.js';
...
// AP Seminar — full teaching content viewer.
...
export default function APSeminarContent() {
  return (
    <PasswordGate
      title="AP Seminar"
...
function APSeminarViewer() {
  ...
  const unit = useMemo(
    () => APSEMINAR_UNITS.find((u) => u.number === selectedUnitNum) || APSEMINAR_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APSEMINAR_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Seminar — full content"
      subtitle={`${APSEMINAR_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Seminar', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apseminar/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APSEMINAR_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apseminar', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apseminar', subunit)} />}

...
export default function APUSGovWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APUSGOV_UNITS.find((u) => String(u.number) === String(unitNum)) || APUSGOV_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apusgov" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP US Government · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP US Government · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

...
import { APCALCAB_UNITS } from '../data/ap-content/apcalcab.js';
import { APCALCAB_FIGURES } from '../data/ap-content/apcalcab-figures.js';
...
// AP Calculus AB — full teaching content viewer.
...
export default function APCalcABContent() {
  return (
    <PasswordGate
...
function APCalcABViewer() {
  ...
  const unit = useMemo(
    () => APCALCAB_UNITS.find((u) => u.number === selectedUnitNum) || APCALCAB_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APCALCAB_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Calculus AB — full content"
      subtitle={`${APCALCAB_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Calculus AB', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apcalcab/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APCALCAB_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apcalcab', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apcalcab', subunit)} />}

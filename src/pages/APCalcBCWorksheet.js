...
export default function APCalcBCWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APCALCBC_UNITS.find((u) => String(u.number) === String(unitNum)) || APCALCBC_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apcalcbc" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Calculus BC · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Calculus BC · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

...
import { APES_UNITS } from '../data/ap-content/apes.js';
import { APES_FIGURES } from '../data/ap-content/apes-figures.js';
...
// AP Environmental Science — full teaching content viewer.
...
export default function APESContent() {
  return (
    <PasswordGate
...
function APESViewer() {
  ...
  const [selectedUnitNum, setSelectedUnitNumRaw] = useState(
    APES_UNITS.find((u) => u.number === initialUnit) ? initialUnit : 1,
  );
  ...
  const unit = useMemo(
    () => APES_UNITS.find((u) => u.number === selectedUnitNum) || APES_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APES_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Environmental Science — full content"
      subtitle={`${APES_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: Terra Council teacher portal.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Environmental Science', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apes/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APES_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apes', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apes', subunit)} />}

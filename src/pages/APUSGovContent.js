...
import { APUSGOV_UNITS } from '../data/ap-content/apusgov.js';
import { APUSGOV_FIGURES } from '../data/ap-content/apusgov-figures.js';
...
// AP US Government — full teaching content viewer.
...
export default function APUSGovContent() {
  return (
    <PasswordGate
...
function APUSGovViewer() {
  ...
  const unit = useMemo(
    () => APUSGOV_UNITS.find((u) => u.number === selectedUnitNum) || APUSGOV_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APUSGOV_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP US Government — full content"
      subtitle={`${APUSGOV_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP US Government', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apusgov/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APUSGOV_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apusgov', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apusgov', subunit)} />}

...
export default function APPhysCEMWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APPHYSCEM_UNITS.find((u) => String(u.number) === String(unitNum)) || APPHYSCEM_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apphyscem" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Physics C: E&M · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Physics C: E&M · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

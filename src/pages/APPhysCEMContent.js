...
import { APPHYSCEM_UNITS } from '../data/ap-content/apphyscem.js';
import { APPHYSCEM_FIGURES } from '../data/ap-content/apphyscem-figures.js';
...
// AP Physics C: E&M — full teaching content viewer.
...
export default function APPhysCEMContent() {
  return (
    <PasswordGate
...
function APPhysCEMViewer() {
  ...
  const unit = useMemo(
    () => APPHYSCEM_UNITS.find((u) => u.number === selectedUnitNum) || APPHYSCEM_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APPHYSCEM_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Physics C: E&M — full content"
      subtitle={`${APPHYSCEM_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Physics C: E&M', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apphyscem/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APPHYSCEM_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apphyscem', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apphyscem', subunit)} />}

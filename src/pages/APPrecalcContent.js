...
import { APPRECALC_UNITS } from '../data/ap-content/apprecalc.js';
import { APPRECALC_FIGURES } from '../data/ap-content/apprecalc-figures.js';
...
// AP Precalculus — full teaching content viewer.
...
export default function APPrecalcContent() {
  return (
    <PasswordGate
...
function APPrecalcViewer() {
  ...
  const unit = useMemo(
    () => APPRECALC_UNITS.find((u) => u.number === selectedUnitNum) || APPRECALC_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APPRECALC_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Precalculus — full content"
      subtitle={`${APPRECALC_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Precalculus', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apprecalc/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APPRECALC_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apprecalc', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apprecalc', subunit)} />}

...
export default function APPhys2Worksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APPHYS2_UNITS.find((u) => String(u.number) === String(unitNum)) || APPHYS2_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apphys2" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Physics 2 · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Physics 2 · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

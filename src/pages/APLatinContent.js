...
import { APLATIN_UNITS } from '../data/ap-content/aplatin.js';
import { APLATIN_FIGURES } from '../data/ap-content/aplatin-figures.js';
...
// AP Latin — full teaching content viewer.
...
export default function APLatinContent() {
  return (
    <PasswordGate
      title="AP Latin"
...
function APLatinViewer() {
  ...
  const unit = useMemo(
    () => APLATIN_UNITS.find((u) => u.number === selectedUnitNum) || APLATIN_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APLATIN_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Latin — full content"
      subtitle={`${APLATIN_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Latin', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/aplatin/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APLATIN_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('aplatin', subunit)) && <VideoBlock video={subunit.video || resolveVideo('aplatin', subunit)} />}

...
import { APUSH_UNITS } from '../data/ap-content/apush.js';
import { APUSH_FIGURES } from '../data/ap-content/apush-figures.js';
...
// AP US History — full teaching content viewer.
...
export default function APUSHContent() {
  return (
    <PasswordGate
      title="AP US History"
...
function APUSHViewer() {
  ...
  const unit = useMemo(
    () => APUSH_UNITS.find((u) => u.number === selectedUnitNum) || APUSH_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APUSH_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP US History — full content"
      subtitle={`${APUSH_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP US History', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apush/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APUSH_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apush', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apush', subunit)} />}

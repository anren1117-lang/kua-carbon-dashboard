...
export default function APSpanishWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APSPANISH_UNITS.find((u) => String(u.number) === String(unitNum)) || APSPANISH_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apspanish" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Spanish Language and Culture · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Spanish Language and Culture · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

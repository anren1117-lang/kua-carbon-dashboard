...
import { APEURO_UNITS } from '../data/ap-content/apeuro.js';
import { APEURO_FIGURES } from '../data/ap-content/apeuro-figures.js';
...
// AP European History — full teaching content viewer.
...
export default function APEuroContent() {
  return (
    <PasswordGate
...
function APEuroViewer() {
  ...
  const unit = useMemo(
    () => APEURO_UNITS.find((u) => u.number === selectedUnitNum) || APEURO_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APEURO_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP European History — full content"
      subtitle={`${APEURO_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP European History', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apeuro/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APEURO_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apeuro', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apeuro', subunit)} />}

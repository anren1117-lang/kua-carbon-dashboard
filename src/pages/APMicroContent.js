...
import { APMICRO_UNITS } from '../data/ap-content/apmicro.js';
import { APMICRO_FIGURES } from '../data/ap-content/apmicro-figures.js';
...
// AP Microeconomics — full teaching content viewer.
...
export default function APMicroContent() {
  return (
    <PasswordGate
...
function APMicroViewer() {
  ...
  const unit = useMemo(
    () => APMICRO_UNITS.find((u) => u.number === selectedUnitNum) || APMICRO_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APMICRO_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Microeconomics — full content"
      subtitle={`${APMICRO_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Microeconomics', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apmicro/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APMICRO_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apmicro', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apmicro', subunit)} />}

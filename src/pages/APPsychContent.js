...
import { APPSYCH_UNITS } from '../data/ap-content/appsych.js';
import { APPSYCH_FIGURES } from '../data/ap-content/appsych-figures.js';
...
// AP Psychology — full teaching content viewer.
...
export default function APPsychContent() {
  return (
    <PasswordGate
      title="AP Psychology"
...
function APPsychViewer() {
  ...
  const unit = useMemo(
    () => APPSYCH_UNITS.find((u) => u.number === selectedUnitNum) || APPSYCH_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APPSYCH_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Psychology — full content"
      subtitle={`${APPSYCH_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Psychology', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/appsych/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APPSYCH_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('appsych', subunit)) && <VideoBlock video={subunit.video || resolveVideo('appsych', subunit)} />}

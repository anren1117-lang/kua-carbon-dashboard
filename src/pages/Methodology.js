...
const sources = [
  ...
  { domain: 'Electricity', source: 'ISO New England Electric Generator Air Emissions Report (2024 data) + EPA eGRID NEWE per-fuel rates', use: 'Per-fuel output-basis kg/kWh (combined-cycle gas 0.40, oil 0.78, coal 0.95, imports 0.30) summed across the 2024 generation mix → effective ~0.234 kg/kWh. ISO-NE separately publishes a regional average for all in-region generation — 597 lb CO₂/MWh for 2024, down from 643 in 2022; we report against the per-fuel output-basis reconstruction instead, because it tracks how building-level kWh actually maps to emissions.' },

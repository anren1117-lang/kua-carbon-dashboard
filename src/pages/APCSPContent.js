...
import { APCSP_FIGURES } from '../data/ap-content/apcsp-figures.js';
...
// AP Computer Science Principles — full teaching content viewer.
...
export default function APCSPContent() {
  return (
    <PasswordGate
...
function APCSPViewer() {
  ...
  const unit = useMemo(
    () => APCSP_UNITS.find((u) => u.number === selectedUnitNum) || APCSP_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APCSP_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Computer Science Principles — full content"
      subtitle={`${APCSP_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Computer Science Principles', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apcsp/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APCSP_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apcsp', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apcsp', subunit)} />}

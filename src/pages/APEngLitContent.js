...
import { APENGLIT_UNITS } from '../data/ap-content/apenglit.js';
import { APENGLIT_FIGURES } from '../data/ap-content/apenglit-figures.js';
...
// AP English Literature — full teaching content viewer.
...
export default function APEngLitContent() {
  return (
    <PasswordGate
...
function APEngLitViewer() {
  ...
  const unit = useMemo(
    () => APENGLIT_UNITS.find((u) => u.number === selectedUnitNum) || APENGLIT_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APENGLIT_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP English Literature — full content"
      subtitle={`${APENGLIT_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP English Literature', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apenglit/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APENGLIT_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apenglit', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apenglit', subunit)} />}

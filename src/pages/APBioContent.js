...
import { APBIO_UNITS } from '../data/ap-content/apbio.js';
import { APBIO_FIGURES } from '../data/ap-content/apbio-figures.js';
...
// AP Biology — full teaching content viewer.
...
export default function APBioContent() {
  return (
    <PasswordGate
      title="AP Biology"
...
function APBioViewer() {
  ...
  const unit = useMemo(
    () => APBIO_UNITS.find((u) => u.number === selectedUnitNum) || APBIO_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APBIO_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Biology — full content"
      subtitle={`${APBIO_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Biology', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apbio/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APBIO_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apbio', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apbio', subunit)} />}
...
// Embeds a Khan Academy, CrashCourse, or other named-source video for the
// subunit. Renders the URL as a labeled link plus an iframe when the URL is
// embeddable. Teachers populate URLs in the data files (videos.js or the
// per-subunit `video` field).
...
// Convert a YouTube watch / playlist URL to its embeddable form. Other
// providers (Khan Academy, Vimeo) pass through their normal URL; the iframe
// gracefully degrades when the source doesn't allow embedding.

...
export default function APHugWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APHUG_UNITS.find((u) => String(u.number) === String(unitNum)) || APHUG_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/aphug" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Human Geography · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Human Geography · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

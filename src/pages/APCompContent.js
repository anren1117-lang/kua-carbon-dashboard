...
import { APCOMP_UNITS } from '../data/ap-content/apcomp.js';
import { APCOMP_FIGURES } from '../data/ap-content/apcomp-figures.js';
...
// AP Comparative Government — full teaching content viewer.
...
export default function APCompContent() {
  return (
    <PasswordGate
...
function APCompViewer() {
  ...
  const unit = useMemo(
    () => APCOMP_UNITS.find((u) => u.number === selectedUnitNum) || APCOMP_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APCOMP_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Comparative Government — full content"
      subtitle={`${APCOMP_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Comparative Government', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apcomp/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APCOMP_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apcomp', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apcomp', subunit)} />}

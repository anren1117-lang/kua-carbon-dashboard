...
import { APART_UNITS } from '../data/ap-content/apart.js';
import { APART_FIGURES } from '../data/ap-content/apart-figures.js';
...
// AP Art History — full teaching content viewer.
...
function APArtViewer() {
  ...
  const [selectedUnitNum, setSelectedUnitNumRaw] = useState(
    APART_UNITS.find((u) => u.number === initialUnit) ? initialUnit : 1,
  );
  ...
  const unit = useMemo(
    () => APART_UNITS.find((u) => u.number === selectedUnitNum) || APART_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APART_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Art History — full content"
      subtitle={`${APART_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Art History', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apart/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APART_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apart', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apart', subunit)} />}

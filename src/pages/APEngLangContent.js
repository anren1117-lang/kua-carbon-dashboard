...
import { APENGLANG_UNITS } from '../data/ap-content/apenglang.js';
import { APENGLANG_FIGURES } from '../data/ap-content/apenglang-figures.js';
...
// AP English Language — full teaching content viewer.
...
export default function APEngLangContent() {
  return (
    <PasswordGate
...
function APEngLangViewer() {
  ...
  const unit = useMemo(
    () => APENGLANG_UNITS.find((u) => u.number === selectedUnitNum) || APENGLANG_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APENGLANG_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP English Language — full content"
      subtitle={`${APENGLANG_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP English Language', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apenglang/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APENGLANG_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apenglang', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apenglang', subunit)} />}

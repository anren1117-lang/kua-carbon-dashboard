...
import { APITALIAN_UNITS } from '../data/ap-content/apitalian.js';
import { APITALIAN_FIGURES } from '../data/ap-content/apitalian-figures.js';
...
// AP Italian Language and Culture — full teaching content viewer.
...
export default function APItalianContent() {
  return (
    <PasswordGate
...
function APItalianViewer() {
  ...
  const unit = useMemo(
    () => APITALIAN_UNITS.find((u) => u.number === selectedUnitNum) || APITALIAN_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APITALIAN_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP Italian Language and Culture — full content"
      subtitle={`${APITALIAN_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP Italian Language and Culture', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apitalian/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APITALIAN_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apitalian', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apitalian', subunit)} />}

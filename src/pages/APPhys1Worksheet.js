...
export default function APPhys1Worksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APPHYS1_UNITS.find((u) => String(u.number) === String(unitNum)) || APPHYS1_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apphys1" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Physics 1 · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Physics 1 · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

...
import { APAFAM_UNITS } from '../data/ap-content/apafam.js';
import { APAFAM_FIGURES } from '../data/ap-content/apafam-figures.js';
...
// AP African American Studies — full teaching content viewer.
...
export default function APAfAmContent() {
  return (
    <PasswordGate
...
function APAfAmViewer() {
  ...
  const unit = useMemo(
    () => APAFAM_UNITS.find((u) => u.number === selectedUnitNum) || APAFAM_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APAFAM_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP African American Studies — full content"
      subtitle={`${APAFAM_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP African American Studies', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apafam/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APAFAM_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apafam', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apafam', subunit)} />}

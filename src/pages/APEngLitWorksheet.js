...
export default function APEngLitWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APENGLIT_UNITS.find((u) => String(u.number) === String(unitNum)) || APENGLIT_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apenglit" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP English Literature · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP English Literature · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

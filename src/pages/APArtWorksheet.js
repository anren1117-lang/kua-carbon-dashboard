...
export default function APArtWorksheet() {
  const { unitNum } = useParams();
  const unit = useMemo(
    () => APART_UNITS.find((u) => String(u.number) === String(unitNum)) || APART_UNITS[0],
    [unitNum],
  );
  ...
  return (
    <div style={styles.page} className="kua-worksheet-page">
      ...
      <div className="no-print" style={styles.toolbar}>
        <Link to="/teacher/ap/apart" style={styles.toolbarBack}>
      ...
      <article style={styles.sheet} className="kua-worksheet-sheet">
        <header style={styles.head}>
          <div style={styles.headLine1}>AP Art History · Unit {unit.number}</div>
        ...
        <footer style={styles.footer}>
          AP Art History · Unit {unit.number} · {cleanTitle} · KUA Carbon Dashboard
        </footer>
      </article>

...
import { APGERMAN_UNITS } from '../data/ap-content/apgerman.js';
import { APGERMAN_FIGURES } from '../data/ap-content/apgerman-figures.js';
...
// AP German Language and Culture — full teaching content viewer.
...
export default function APGermanContent() {
  return (
    <PasswordGate
...
function APGermanViewer() {
  ...
  const unit = useMemo(
    () => APGERMAN_UNITS.find((u) => u.number === selectedUnitNum) || APGERMAN_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APGERMAN_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP German Language and Culture — full content"
      subtitle={`${APGERMAN_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP German Language and Culture', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apgerman/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APGERMAN_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apgerman', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apgerman', subunit)} />}

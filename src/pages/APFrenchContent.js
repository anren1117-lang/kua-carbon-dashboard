...
import { APFRENCH_UNITS } from '../data/ap-content/apfrench.js';
import { APFRENCH_FIGURES } from '../data/ap-content/apfrench-figures.js';
...
// AP French Language and Culture — full teaching content viewer.
...
export default function APFrenchContent() {
  return (
    <PasswordGate
...
function APFrenchViewer() {
  ...
  const unit = useMemo(
    () => APFRENCH_UNITS.find((u) => u.number === selectedUnitNum) || APFRENCH_UNITS[0],
    [selectedUnitNum],
  );

  const totalSubunits = useMemo(
    () => APFRENCH_UNITS.reduce((sum, u) => sum + (u.subunits?.length || 0), 0),
    [],
  );
  ...
  return (
    <ModulePage
      title="AP French Language and Culture — full content"
      subtitle={`${APFRENCH_UNITS.length} units · ${totalSubunits} subunits · read-to-learn teaching material. Original authoring: KUA Carbon Dashboard.`}
      toolbar={
...
function UnitView({ unit }) {
  ...
  const handlePPT = async () => {
    ...
    try {
      ...
      await downloadUnitPPT({ course: 'AP French Language and Culture', unit });
    } catch (err) {
    ...
    <div>
      <header style={styles.unitHead}>
        ...
        <div style={styles.artifactStrip}>
          ...
          <Link
            to={`/teacher/ap/apfrench/unit/${unit.number}/worksheet`}
...
function SubunitBlock({ subunit }) {
  ...
  const figures = APFRENCH_FIGURES[subunit.code] || [];
  ...
  return (
    <div style={styles.subunit} id={`s-${subunit.code}`}>
      ...
      {(subunit.video || resolveVideo('apfrench', subunit)) && <VideoBlock video={subunit.video || resolveVideo('apfrench', subunit)} />}

...
**Known inconsistency, deferred:** `Scope2LiveDashboard` annualizes per-building `envysionSnapshot` rows by `SNAPSHOT_ANNUALIZE_FACTOR` (day-spread from `year1Months`), not by `seasonalYearFraction` — a second per-building annualization rule, ~2.8% apart campus-wide. Reconcile it onto this rule before the product ships to a buyer.
